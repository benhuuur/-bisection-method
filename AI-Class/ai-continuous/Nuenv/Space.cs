...
public static class Space
{
    ...
    public static double[] Geometric(double a, double b, int n)
    {
        ...
        var result = new double[n];
        var r = System.Math.Pow(b / a, 1.0 / (n - 1));

        for (int i = 0; i < n; i++)
        {
            result[i] = a * System.Math.Pow(r, i);
        }
    ...
    public static double[] Logarithmic(double a, double b, int n)
    {
        ...
        var result = new double[n];
        var aLog = System.Math.Log(a, baseLog);
        var bLog = System.Math.Log(b, baseLog);

        for (int i = 0; i < n; i++)
        {
            var xLog = aLog + i * (bLog - aLog) / (n - 1);
            result[i] = System.Math.Pow(baseLog, xLog);
        }
